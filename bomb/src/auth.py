...
class Auth:
    ...
    def login(self):
        ...
        if self.actions.clickButton(connect_wallet_button):
            self.log.console('Connect wallet button detected, logging in!', emoji='🎉', color='green')
        ...
        if metamask_unlock_coord is not False:
            ...
            self.log.console('Found unlock button. Waiting for password', emoji='🔓', color='yellow')
            password = metamaskData["password"]
            pyautogui.typewrite(password, interval=0.1)
            self.actions.sleep(1, 2)
            if self.actions.clickButton(metamask_unlock_button):
                self.log.console('Unlock button clicked', emoji='🔓', color='green')
        ...
        if self.recognition.currentScreen() == "main":
            self.log.console('Logged in', services=True, emoji='🎉', color='green')
            return True
        else:
            self.log.console('Login failed, trying again', emoji='😿', color='red')
            login_attempts += 1

            if (login_attempts > 2):
                self.telegram.sendPrint()
                self.log.console('+3 login attempts, retrying', services=True, emoji='🔃', color='red')
                login_attempts = 0
                self.errors.verify()
    ...
    def checkLogout(self):
        ...
        if currentScreen == "unknown" or currentScreen == "login":
            if self.recognition.positions(connect_wallet_button) is not False:
                self.telegram.sendPrint()
                self.log.console('Logout detected', services=True, emoji='😿', color='red')
                self.actions.refreshPage()
                self.login()
            elif self.recognition.positions(metamask_sign_button):
                self.log.console('Sing button detected', services=True, emoji='✔️', color='green')
                if self.actions.clickButton(metamask_cancel_button):
                    self.log.console('Metamask is glitched, fixing', services=True, emoji='🙀', color='yellow')
            else:
                return False
